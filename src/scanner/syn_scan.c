		...
		t_port_state *state)
{
	...
	t_reply_classifier	classify_tcp;
	t_port_state		timeout_state;

	if (type == SCAN_SYN)
	{
		build_probe = build_syn_packet;
		classify_tcp = classify_syn_tcp;
		timeout_state = PORT_FILTERED;
	}
	else if (type == SCAN_ACK)
	{
		build_probe = build_ack_packet;
		classify_tcp = classify_ack_tcp;
		timeout_state = PORT_FILTERED;
	}
	else
		return (-1);
...
{
	return (tcp_scan_port_for_type(SCAN_SYN, sock, p, src, sport,
			dst, dport, timeout_ms, state));
}

int	scan_port_by_type(t_scan_type type, int sock, pcap_t *p,
		struct in_addr src, uint16_t sport,
		struct in_addr dst, uint16_t dport,
		uint32_t timeout_ms, t_port_state *state)
{
	if (type == SCAN_SYN)
		return (syn_scan_port(sock, p, src, sport, dst, dport,
				timeout_ms, state));
	if (type == SCAN_ACK)
		return (ack_scan_port(sock, p, src, sport, dst, dport,
				timeout_ms, state));
	return (-1);
}
