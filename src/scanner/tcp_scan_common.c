...
static t_port_state	default_state_for_scan(t_scan_type scan_type)
{
		...
		t_scan_type scan_type, t_port_state *state)
{
	...
	len = build_tcp_packet(buf, src, dst, sport, dport, flags);
	...
	*state = default_state_for_scan(scan_type);
